...
{
	public Camera CC; // Allows access to camera (to shoot detection ray)
	public Text pT; // Allows us access to "Selected piece" text
	public GameObject LH; // Access to the ListHolder (to turn the data panel on and off)

	public GameObject ES;

	string hName = ""; // Name of heart piece that is highlighted
	...
	void Update(){
		...
		if (Input.GetMouseButtonDown (0) && Compare.first!=0 && !UnityEngine.EventSystems.EventSystem.current.IsPointerOverGameObject()) { // and not over a UI element?
			
			Ray ray = CC.ScreenPointToRay(Input.mousePosition);
			RaycastHit hit;

			if (Physics.Raycast (ray, out hit)) { // If it hits a piece of heart

				hName = hit.transform.name;

				if (Compare.first==1 && !Compare.pieces2.Contains (hName)) { // Green
				...
				if (Compare.first==2 && !Compare.pieces1.Contains (hName)) { // Red
	...
	*/
	...
	public void Reset(){ // Reset text and put any out pieces in
