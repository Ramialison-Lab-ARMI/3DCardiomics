...
public class PanelScript : MonoBehaviour {
	...
	public GameObject col1; // Our template column1 cell (The text)
	public GameObject col2; // Our template column2 cell (The button)
	public GameObject LH;
	public GameObject SH; // Access to the script holder (to use 'Colour' class methods)
	public Scrollbar SB;
	...
	private void colBtnActions (string gName, int gId) { // Colours and generates table from geneId 
	...
	private void moreBtnActions () { // Adds 20 more genes to the distance table
	...
	private void add20(bool more = false){
		...
		for (int i=upto; i < upto+rows; i++) {
			...
			btn.name = geneName;
			btn.onClick.AddListener(delegate { colBtnActions( geneName,i); });
		...
		if (upto < 400 && upto<Colour.valuesCount-1) {
			...
			moreBtn.onClick.AddListener(delegate { moreBtnActions(); });
