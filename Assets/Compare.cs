...
public class Compare : MonoBehaviour {
	...
	private void colBtnActions (string gName) { // Colours and generates table from geneId 
	...
	private void add20(bool more = false){
		...
		if (more) { // Destroy the more button
		...
		for (int i=upto; i < upto+rows; i++) {
			...
			btn.name = geneName;
			btn.onClick.AddListener(delegate { colBtnActions( geneName); });
